...
const StoreDB = {
  ...
  async ChannelList(ChannelList = undefined, isClear = false) {
    ...
    if (ChannelList) {
      return AsyncStorage.setItem(CHANNELLIST_DATA, JSON.stringify(ChannelList));
    }
    let savedData = await AsyncStorage.getItem(CHANNELLIST_DATA);
    return savedData ? JSON.parse(savedData) : {};
  },

  async nurseAppointmentData(nurseAppointmentData = undefined, isClear = false) {
    if (nurseAppointmentData) {
      return AsyncStorage.setItem(NURSE_APPOINTMENT_DATA, JSON.stringify(nurseAppointmentData));
    }
